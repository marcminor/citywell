				...
				<?php if (has_post_thumbnail($post->ID)){ ?>
					...
					<div class="post-content<?php if (!has_post_thumbnail($post->ID)){ ?> no-thumb<?php } ?>">
						...
						<?php if (of_get_option('js_hide_metainfo') == 0){
							...
							<p class="post-meta"><?php _e('Posted on','savior'); ?> <strong><?php the_time('F j, Y') ?></strong> <?php _e('by','savior'); ?> <?php echo '<a href="'.home_url().'/author/'.get_the_author_meta( 'user_nicename' , $author_id ).'/">'; the_author_meta( 'display_name' , $author_id ); echo '</a>'; ?> <?php _e('in','savior'); ?>
						...
						<a href="<?php echo get_permalink($post->ID) ?>" class="more"><?php _e('Continue Reading','savior'); ?></a>
					</div>
