...
if (version_compare(PHP_VERSION, '5', '<')) {
	...
	$out .= sprintf("<p><strong>Your PHP version is '%s'.<br>The Event Calendar requires PHP 5 or higher.</strong></p><p>Ask your web host how to enable PHP 5 on your site.</p>", PHP_VERSION);
	$out .= "</div>";
	print $out;
}

define('AEC_MENU_POSITION', '25.6');
define('AEC_VERSION', '1.0.2');
define('AEC_FILE', basename(__FILE__));
define('AEC_NAME', str_replace('.php', '', AEC_FILE));
define('AEC_PATH', get_template_directory().'/_theme_settings/plugins/ajax-event-calendar/');
define('AEC_URL', get_template_directory_uri().'/_theme_settings/plugins/ajax-event-calendar/');
...
function ajax_event_calendar_is_installed() {
	global $wpdb;
	$res = $wpdb->get_var('SHOW TABLES LIKE "' . $wpdb->prefix . AEC_EVENT_TABLE . '"');
	return $res !== null;
}


if (isset($_GET['aec_calendar_db_upgaded'])) {
	function aec_installed() {
		$message = '<strong>Awesome!</strong> The events functionality is now active. Click on "Events" in the sidebar to add events.';
		echo '<div class="message updated"><p>' . $message . '</p></div>';
	}
	add_action('admin_notices', 'aec_installed');
}


function aec_show_events_installation_message() {
	$upgrade_url = get_admin_url() . '?savior-aec-upgrade-database=true';
	$message = '<strong>Need to Add Events?</strong> You need to upgrade your database in order to add the event functionality to this theme. <a href="' . $upgrade_url . '">Upgrade Now!</a>';
	echo '<div class="message updated"><p>' . $message . '</p></div>';
}

if (!ajax_event_calendar_is_installed() && !isset($_GET['savior-aec-upgrade-database'])) {
	if (is_admin()) {
		add_action('admin_notices', 'aec_show_events_installation_message');
	}
	return;
}
...
if (!class_exists('ajax_event_calendar')) {
	...
	class ajax_event_calendar {
		...
		// protect the constructor with a singleton. Creating new instance
		// of the class will lead to double initialization
		private static $_instance = null;
		public static function get_instance() {
			if (self::$_instance === null) {
				self::$_instance = new ajax_event_calendar();
			}
		...
		function install() {
			...
			if ($wpdb->get_var('SHOW TABLES LIKE "' . $wpdb->prefix . AEC_CATEGORY_TABLE . '"') != $wpdb->prefix . AEC_CATEGORY_TABLE) {
				$sql = 'CREATE TABLE ' . $wpdb->prefix . AEC_CATEGORY_TABLE . ' (
						...
						VALUES 	(NULL, "Events", "0988ba", "FFFFFF");';
				dbDelta($sql);
			}
	    ...
	    function localize_plugin($page) {
			...
			$path = AEC_PATH . 'locale';
			$loaded = load_theme_textdomain(AEC_NAME, $path);
		...
		function render_admin_menu() {
			if (function_exists('add_options_page')) {
				...
				$page = add_menu_page('Events',  __('Events', AEC_NAME), 'aec_add_events', AEC_FILE, array($this, 'render_admin_calendar'), AEC_URL . 'css/images/menu_icon_events.png', AEC_MENU_POSITION);
				...
				if (current_user_can('aec_manage_calendar')) {
					...
					//$sub_report = add_submenu_page(AEC_FILE, 'Reports', __('Reports', AEC_NAME), 'aec_manage_calendar', 'aec_activity_report', array($this, 'render_activity_report'));
		...
		function localized_variables() {
			...
			// Prevent infinite loop in the javascript timepicker when the settings are missing
			// upon initial plugin installation
			if (empty($options['step_interval'])) {
				$options['step_interval'] = 30;
			}
			...
			return array(
				...
				'show_weekends'				=> (isset($options['show_weekends']) ? $options['show_weekends'] : false),
				...
				'fri'						=> __('Fri', AEC_NAME),
				'sat'						=> __('Sat', AEC_NAME),
				'close_event_form'			=> __('Close Event', AEC_NAME),
				'loading_event_form'		=> __('Loading event...', AEC_NAME),
		...
		function render_admin_calendar() {
			...
			$out .= "<h2><span class='em-icon icon32'></span>" . __('Event Calendar', AEC_NAME) . "</h2>\n";
			$out .= "<div id='aec-calendar' class='centered-title'></div>\n";
		...
		function render_shortcode_calendar($atts) {
			...
			$out .= "</div>\n";
			$out .= "<div id='aec-calendar' ".($cat_count < 2 ? "class='no-cats'" : "")."></div>\n";
			$out .= $this->render_aec_version();
			$out .= "</div>\n";
			...
			echo '<div class="hidden-responsive-calendar">';
			echo js_display_upcoming($atts);
			echo '</div>';
			...
			if (!empty($event_list)):
				...
				foreach($event_list as $event):
					...
					<a href="#" class="event-link" onclick="jQuery.aecDialog({'id':<?php echo $event['id']; ?>,'start':'<?php echo $event['start']; ?>','end':'<?php echo $event['end']; ?>'}); return false;"><span class="icon"><img src="<?php echo get_template_directory_uri(); ?>/_theme_styles/images/clock.png" alt="" /></span> <span class="right">
						...
						<?php else : ?>
							<?php echo date($date_format,$start_date); if ($same_day): echo ' - '.date($date_format_b,$end_date); endif; ?>
						<?php endif; ?>
		...
		function render_admin_category() {
			...
			$top = $this->add_wrap(__('Categories', AEC_NAME), "<h2><span class='em-icon icon32'></span>", "</h2>");
		...
		function render_activity_report() {
			...
			$top = $this->add_wrap(__('Reports', AEC_NAME), "<h2><span class='em-icon icon32'></span>", "</h2>");
...
if (isset($_GET['savior-aec-upgrade-database']) && !ajax_event_calendar_is_installed()) {
	ajax_event_calendar::install();
	wp_redirect(get_admin_url() . '?aec_calendar_db_upgaded=true');
}
