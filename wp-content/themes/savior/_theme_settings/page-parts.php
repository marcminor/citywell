				...
				if (!empty($event)) { ?>
					...
					<a href="#" id="countdown_link" onclick="jQuery.aecDialog({'id':<?php echo $event['id']; ?>,'start':'<?php echo $event['start']; ?>','end':'<?php echo $event['end']; ?>'}); return false;">
						<div class="shell">
							...
							<?php $event_countdown = ($countdown_pretext == 'DEFAULT' ? __('Featured Event in','savior').' ' : ($countdown_pretext ? $countdown_pretext.' ' : '')).'<span>'.date('F j, Y G i Z',strtotime($event['start'])).'</span>';
				...
				if (!empty($event)) { ?>
					...
					<a href="#" id="countdown_link" onclick="jQuery.aecDialog({'id':<?php echo $event->id; ?>,'start':'<?php echo $event->start; ?>','end':'<?php echo $event->end; ?>'}); return false;">
						<div class="shell">
							...
							<?php $event_countdown = ($countdown_pretext == 'DEFAULT' ? __('Next Event in','savior').' ' : ($countdown_pretext ? $countdown_pretext.' ' : '')).'<span>'.date('F j, Y G i Z',strtotime($event->start)).'</span>';
							$event_title = stripslashes($event->title); ?>
	...
	if ($content):
		...
		<div class="shell">
			...
			<article id="content" <?php post_class($content_post_class); ?>>
				<?php if (is_array($other_options) && !in_array('hide_breadcrumbs',$other_options) || !is_array($other_options)): js_breadcrumbs($post_id); endif; ?>
				<?php if (is_array($other_options) && !in_array('hide_title',$other_options) || !is_array($other_options)): ?><h1><?php the_title(); ?></h1><?php endif; ?>
