	...
	<div id="comments-block">
		<div class="post light">
				...
				<?php if (!empty($_SERVER['SCRIPT_FILENAME']) && 'comments.php' == basename($_SERVER['SCRIPT_FILENAME']))
					die (__('Please do not load this page directly. Thanks!','savior'));
				
				if ( post_password_required() ) { ?>
					<?php _e('This post is password protected. Enter the password to view comments.','savior'); ?>
				<?php
					return;
				}
				...
				if ( have_comments() ) : ?>
					...
					<h2 id="comments" class="dotted-line-title"><span><?php comments_number(__('No Responses','savior'), __('One Response','savior'), '% '.__('Responses','savior'));?></span></h2>
				 ...
				 <?php else : // this is displayed if there are no comments so far ?>
					...
					<?php if ( comments_open() ) : ?>
					 ...
					 <?php else : // comments are closed ?>
						<p class="closed-comments"><?php _e('Comments are closed.','savior'); ?></p>
				...
				<?php if ( comments_open() ) : ?>
					...
					comment_form(array('title_reply'=>'<h2 id="comments" class="dotted-line-title"><span>'.__('Leave a Reply','savior').'</span></h2>','comment_field'=>$comment_field));
